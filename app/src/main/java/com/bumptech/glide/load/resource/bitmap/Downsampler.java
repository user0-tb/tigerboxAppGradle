        ...
        if (options.inSampleSize == 1 || z5) {
            ...
            if (downsampler.shouldUsePool(imageType)) {
                if (i6 < 0 || i7 < 0 || !z2 || !z5) {
                    ...
                    float f2 = i8;
                    float f3 = f;
                    round = Math.round(((int) Math.ceil(i6 / f2)) * f3);
                    round2 = Math.round(((int) Math.ceil(i7 / f2)) * f3);
                    if (Log.isLoggable(TAG, 2)) {
                        Log.v(TAG, "Calculated target [" + round + "x" + round2 + "] for source [" + i6 + "x" + i7 + "], sampleSize: " + i8 + ", targetDensity: " + options.inTargetDensity + ", density: " + options.inDensity + ", density multiplier: " + f3);
                    }
                } else {
                    round = i4;
                    round2 = i5;
