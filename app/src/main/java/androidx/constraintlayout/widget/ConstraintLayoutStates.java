    ...
    private void parseConstraintSet(Context context, XmlPullParser parser) {
        ...
        for (int i = 0; i < attributeCount; i++) {
            ...
            if (attributeName != null && attributeValue != null && TtmlNode.ATTR_ID.equals(attributeName)) {
                int identifier = attributeValue.contains(DownloadsManager.FOLDERS_SEPARATOR) ? context.getResources().getIdentifier(attributeValue.substring(attributeValue.indexOf(47) + 1), TtmlNode.ATTR_ID, context.getPackageName()) : -1;
                if (identifier == -1) {
